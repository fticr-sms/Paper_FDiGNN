...
for node in pathway_to_alter + single_nodes:
    if node == gap_node:
        pos_values = np.random.choice(low_int_values, 237, replace=False)
        ...
        neg_values = np.random.choice(high_int_values, 213, replace=False)
...
for r in datasets:
    ...
    while best_loss > 0.15:
        ...
        print('----------------------------------------')
        print('\n')
        torch.save(best_model.state_dict(), f'MD_TAG_Models/MD_model_Graphlets5OppGap_4TAG_64HC3D001LR0001WD3K_{r}_{sys.argv[-1]}')
