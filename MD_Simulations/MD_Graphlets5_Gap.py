...
with open('Data/MD_GNNPathwayTest_graphlets_size5.txt') as a:
    gs = a.read().split('\n')

with open('Data/MD_GNNPathwayTest_singlenodes_size5.txt') as a:
    sns_p = a.read().split('\n')
...
with open('Data/MD_5Graphlets_gaps.txt') as a:
...
values = np.random.choice(mid_int_values, 177+213+237, replace=False)
bin_values = []
for v in values:
    if v < 0:
        bin_values.append(0)
...
for r in datasets:
    ...
    while best_loss > 0.15:
        ...
        train_loader = DataLoader(pd.Series(all_data_full[r])[train_index].values, batch_size=16, shuffle=True)
        test_loader = DataLoader(pd.Series(all_data_full[r])[test_index].values, batch_size=32, shuffle=False)
        ...
        best_model = FDiGNN_TAG(hidden_channels=64, drop=0.3, n_node_feat=len(node_attrs), K=3).to(device)
        ...
        print(f'Finished model. Time elapsed: {end_time_model - start_time_model}.')
        print(f'Best epoch: {best_epoch}, Best loss: {best_loss}, Corr. Accuracy: {best_accuracy}.')
        print('\n')
        print('----------------------------------------')
        print('\n')
        torch.save(best_model.state_dict(), f'MD_TAG_Models/MD_model_Graphlets5Gap_4TAG_64HC3D001LR0001WD3K_{r}_{sys.argv[-1]}')
