...
with open('Data/LID_GNNPathwayTest_graphlets_size6.txt') as a:
    gs = a.read().split('\n')

with open('Data/LID_GNNPathwayTest_singlenodes_size6.txt') as a:
    sns_p = a.read().split('\n')
...
with open('Data/LID_6Graphlets_gaps.txt') as a:
...
values = np.random.choice(mid_int_values, 177*2, replace=False)
bin_values = []
for v in values:
    if v < 0:
        bin_values.append(0)
...
for r in datasets:
    ...
    model.load_state_dict(torch.load(
        f'LID_TAG_Models/LID_model_Graphlets6Gap_4TAG_64HC15D001LR0001WD_{r}_{sys.argv[-1]}'))
...
all_ranks.to_excel(f'LID_TAG_Results/LID_Graphlets6_Gap_TAG_{sys.argv[-1]}.xlsx')

with open(f'LID_TAG_Results/LID_Graphlets6Gap_All_Predictions_TAG_{sys.argv[-1]}.json', 'w') as f:
