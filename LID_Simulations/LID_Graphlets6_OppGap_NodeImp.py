...
for node in pathway_to_alter + single_nodes:
    if node == gap_node:
        pos_values = np.random.choice(low_int_values, 177, replace=False)
        ...
        neg_values = np.random.choice(high_int_values, 177, replace=False)
...
for r in datasets:
    ...
    model.load_state_dict(torch.load(
        f'LID_TAG_Models/LID_model_Graphlets6OppGap_4TAG_64HC15D001LR0001WD_{r}_{sys.argv[-1]}'))
...
all_ranks.to_excel(f'LID_TAG_Results/LID_Graphlets6_OppGap_TAG_{sys.argv[-1]}.xlsx')

with open(f'LID_TAG_Results/LID_Graphlets6OppGap_All_Predictions_TAG_{sys.argv[-1]}.json', 'w') as f:
