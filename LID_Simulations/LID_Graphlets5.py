...
class FDiGNN_Gat(torch.nn.Module):
    def __init__(self, hidden_channels, heads, drop, n_node_feat, n_edge_feat):
        super(FDiGNN_Gat, self).__init__()
        torch.manual_seed(89356)
        self.conv1 = GATv2Conv(n_node_feat, hidden_channels, heads=heads[0], edge_dim=n_edge_feat, add_self_loops=True)
        self.norm1 = BatchNorm1d(hidden_channels*heads[0])
        self.conv2 = GATv2Conv(hidden_channels*heads[0],
                               hidden_channels, heads=heads[1], edge_dim=n_edge_feat, add_self_loops=True)
        self.norm2 = BatchNorm1d(hidden_channels*heads[1])
        self.conv3 = GATv2Conv(hidden_channels*heads[1],
                               hidden_channels, heads=1, edge_dim=n_edge_feat, add_self_loops=True)
    ...
    def forward(self, x, edge_index, batch, edge_attr, retrieve_steps=False):
        # 1. Obtain node embeddings
        x1, self.last_att_conv1 = self.conv1(x, edge_index, edge_attr=edge_attr, return_attention_weights=True)
        x1_relu = self.leakyrelu1(x1)
        x1_norm = self.norm1(x1_relu)
        x1_drop = F.dropout(x1_norm, p=self.drop, training=self.training)
        x2, self.last_att_conv2 = self.conv2(x1_drop, edge_index, edge_attr=edge_attr, return_attention_weights=True)
        x2_relu = self.leakyrelu2(x2)
        x2_norm = self.norm2(x2_relu)
        x2_drop = F.dropout(x2_norm, p=self.drop, training=self.training)
        x3, self.last_att_conv3 = self.conv3(x2_drop, edge_index, edge_attr=edge_attr, return_attention_weights=True)
        x3_relu = self.leakyrelu3(x3)
        x3_norm = self.norm3(x3_relu)
        x3_drop = F.dropout(x3_norm, p=self.drop, training=self.training)
        # 2. Readout layer
        x_emb = self.pooling(x3_drop, batch)
        # 3. Apply a final classifier
        x_emb1 = F.dropout(x_emb, p=self.drop, training=self.training)
        x_emb1 = self.lin1(x_emb1)
        x_emb1 = self.leakyrelu4(x_emb1)
        x_emb2 = self.lin2(x_emb1)
...
for r in datasets:
    ...
    torch.save(best_model.state_dict(), f'LID_TAG_Models/LID_model_Graphlets5_4TAG_64HC15D001LR0001WD_{r}_{sys.argv[-1]}')
    print(f'Graphlets 5 - {r} {sys.argv[-1]}.')
